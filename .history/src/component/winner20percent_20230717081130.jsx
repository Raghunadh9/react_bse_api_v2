...
import useScripDetails from "./useScripDetails";
...
const Winner = () => {
  ...
  // Function to fetch scrip details for all items in data.Table
  const fetchScripDetails = async () => {
    const details = await Promise.all(
      data.Table.map((item) => useScripDetails(item.scrip_cd))
    );
    return details;
  };

  // State to hold the fetched scrip details
  const [scripDetails, setScripDetails] = useState([]);

  useEffect(() => {
    fetchScripDetails().then((details) => {
      setScripDetails(details);
    });
  }, [data]); // Call this effect whenever data changes
