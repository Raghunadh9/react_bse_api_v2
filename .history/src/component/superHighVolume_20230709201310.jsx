...
const SuperHighVolume = () => {
  ...
  const deleteHandler = (id) => {
    const existingData = data.filter((item) => item.dataName === id);
    if (existingData.length > 0) {
      // Data with the specified id exists, perform deletion
      console.log(id);
      // Perform deletion operation here
    } else {
      console.log("Data not found with the specified id");
    }
  };
