...
const SuperHighVolume = () => {
  ...
  const deleteDataByValue = async (valueToDelete) => {
    const dataRef = ref(db, "reactBse");
    const snapshot = await get(dataRef);

    if (snapshot.exists()) {
      const data = snapshot.val();
      const dataToDelete = Object.entries(data).find(
        ([key, item]) => item.dataName === valueToDelete
      );

      if (dataToDelete) {
        const [key] = dataToDelete;
        const dataItemRef = ref(db, `reactBse/${key}`);
        ...
        console.log("No data found with the specified value");
      }
    } else {
        ...
        {bseData.map((i, index) => (
          <tbody key={index + 1}>
            ...
            <tr className="border border-black p-2">
              ...
              <td>
                <button
                  onClick={() => deleteDataByValue(i.scrip_cd)}
                  className="bg-purple-500 p-1 rounded-md text-white"
                >
                  D
                </button>
