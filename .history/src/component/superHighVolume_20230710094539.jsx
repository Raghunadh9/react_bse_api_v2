...
const SuperHighVolume = () => {
  ...
  const deleteHandler = (scrip_cd) => {
    const nodeRef = ref(db, "reactBse/"); // Assuming the root reference of your database is '/'
    const valueToSearch = scrip_cd;

    // Create a query to find the node with the specified value
    const valueQuery = query(nodeRef, orderByValue(), equalTo(valueToSearch));

    // Execute the query to get the matching node
    get(valueQuery)
      .then((snapshot) => {
        if (snapshot.exists()) {
          const childKey = Object.keys(snapshot.val())[0]; // Get the key of the matching node
          const nodeToRemoveRef = ref(db, `/${childKey}`);

          // Remove the node with the specified key
          remove(nodeToRemoveRef)
            .then(() => {
              alert("Remove Succeeded!");
            })
            .catch((err) => console.log(err));
        } else {
          alert("No node found with the specified value.");
        }
      })
      .catch((err) => console.log(err));
  };
