...
const Winner = () => {
  ...
  const removeRef = useRef(false); // Ref to control whether to remove the <tr> element
    ...
    <div>
      ...
      <table className="mt-4 table w-full p-4">
        ...
        {data.Table.sort((a, b) => (a.trd_vol < b.trd_vol ? 1 : -1)).map(
          (i, index) => {
            // Determine if the <tr> should be removed
            removeRef.current = i.trd_vol === 5;

            return (
              <tbody key={index}>
                {!removeRef.current && ( // Conditionally render the <tr> element based on the removeRef value
