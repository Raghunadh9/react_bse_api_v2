...
const SuperHighVolume = () => {
  ...
  const deleteHandler = (scrip_cd) => {
    const nodeRef = ref(db, "reactBse/");
    const keys = Object.keys(data);
    // keys.forEach((key) => {
    //   if (data[key].dataName !== scrip_cd) {
    //     console.log("dataName", data[key].dataName, "scripcd", scrip_cd);
    //   }

    //   // console.log(data[key].dataName);
    // });
    keys.filter((key) => (data[key] === scrip_cd ? alert("yes") : alert("no")));
    ...
    //   if (keyToRemove) {
    //     const nodeToRemoveRef = ref(db, `reactBse/${keyToRemove}`);
