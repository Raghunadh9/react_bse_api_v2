        ...
        Promise.all(requests)
          .then((responses) => {
            ...
            responses.forEach((response) => {
              setBseData(response.data.Table[0]); // Assuming you want to log the response data
            });
          })
          .catch((error) => {
            console.error(error);
  ...
  return (
    <div>
      ...
      <table>
        ...
        <tbody key={bseData.Name}>
          {/* item.scrip_cd */}
          <tr className="border border-black p-2">
            <td> 1</td>
            <td className="border border-black p-2">{bseData.Name}</td>
            <td className="border border-black p-2">{bseData.LTP}</td>
            <td className="border border-black p-2">{bseData.Change}</td>
            <td className="border border-black p-2">
              ...
              >
                <a
                  href={`https://in.tradingview.com/chart/qmDo3C1P/?symbol=BSE%3A${bseData.Name}`}
                  target="_blank"
                ...
                <a
                  href={`https://in.tradingview.com/chart/VViXq6AO/?symbol=BSE%3A${bseData.Name}`}
                  target="_blank"
                  rel="noopener noreferrer"
                >
                ...
                <a
                  href={`https://in.tradingview.com/chart/1gQFO9Ge/?symbol=BSE%3A${bseData.Name}`}
                  target="_blank"
                  rel="noopener noreferrer"
                >
                ...
                <a
                  href={`https://in.tradingview.com/chart/3QIPa6zQ/?symbol=BSE%3A${bseData.Name}`}
                  target="_blank"
                  rel="noopener noreferrer"
                >
            ...
            <td className="border border-black p-2">
              {" "}
              <a
                href={`https://www.google.com/search?q=moneycontrol+${bseData.Name.split(
                  " - "
                )[0]
            ...
            <td className="border border-black p-2">
              <GetVolume scrip_cd={bseData.scrip_cd} />
            </td>
            <td className="border border-black p-2">
              <GetDetailsComponent scrip_cd={bseData.scrip_cd} />
              {/* {bseData.scrip_cd} */}
            </td>
            <td>
              <button
                onClick={() => deleteHandler(bseData.scrip_cd)}
                className="bg-purple-500 p-1 rounded-md text-white"
              >
                D
              </button>
