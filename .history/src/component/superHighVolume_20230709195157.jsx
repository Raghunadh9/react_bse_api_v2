...
const SuperHighVolume = () => {
  ...
  const deleteHandler = (dataName) => {
    const dataRef = ref(db, "reactBse");
    const query = orderByChild(dataRef, "dataName");
    const filteredQuery = query.equalTo(dataName);
    onChildAdded(filteredQuery, (snapshot) => {
      const childRef = ref(db, `reactBse/${snapshot.key}`);
      remove(childRef)
        .then(() => {
