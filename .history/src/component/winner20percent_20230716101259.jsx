...
const fetchAllScripDetails = (scrip_cds) => {
  const requests = scrip_cds.map((scrip_cd) =>
    axios.get(
      `https://bse-api-server.vercel.app/getDetails?scripcode=${scrip_cd}`
    )
  );
  return Promise.all(requests).then((responses) =>
    responses.map((res) => res.data.PBpcUC)
  );
};
...
const Winner = ({ data }) => {
  ...
  useEffect(() => {
    fetchAllScripDetails(data.Table.map((item) => item.scrip_cd)).then(
      (details) => {
          ...
          (i, index) => (
            <tbody key={index}>
              <tr className={`border border-black`}>
                ...
                <td className="border border-black">{/* Popup code */}</td>
